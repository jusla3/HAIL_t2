...
_qchero.each = function (str, callback) {
	var a = [];
	...
	return a;//_qchero(elms);
	//alert(str);
}
_qchero.html = function (html) {
...
_qchero._ = function(str){
	...
	if(elms.length) {
		...
		for(var i = 0;i<elms.length;i++) {
			a.push(_qchero(elms[i]));
		};
		a.on = elms.on;
	}
	return a;
}
_qchero.find = function(a, b){
	var maches = [];
	var query = "";
	if(typeof a != 'object')
	...
	if(a.length) {
		for(var i = 0;i<a.length;i++) {
			var clas = _qchero(a[i]).getAttr('class');
			if(clas) {
				clas = clas.split(" ");
				clas = clas[0];			
				clas = "."+clas;
			...
			var id = _qchero(a[i]).getAttr('id');
			var nodename = _qchero(a[i]).nodeName.toLowerCase();
			var pclas = _qchero(a[i].parentNode).getAttr('class');
			if(pclas) {
				pclas = pclas.split(" ");
				pclas = pclas[0];			
				pclas = "."+pclas;
			...
			var pid = _qchero(a[i].parentNode).getAttr('id');
			var pnodename = _qchero(a[i].parentNode).nodeName.toLowerCase();
			if(pid) pid = "#"+pid; 
				else pid = "";
			if(id) id = "#"+id; 
				else id="";
			...
			if(d.length) {
				for(var i = 0;i<d.length;i++) {
					maches.push(_qchero(d[i]));
				}
			}
			//maches = maches.concat(document.querySelectorAll(query));	
		}		
	} else {
		var clas = _qchero(a).getAttr('class');
			if(clas) {
				clas = clas.split(" ");
				clas = clas[0];			
				clas = "."+clas;
			}		
		var id = _qchero(a).getAttr('id');
		var nodename = _qchero(a).nodeName.toLowerCase();
		var pclas = _qchero(a.parentNode).getAttr('class');
			if(pclas) {
				pclas = pclas.split(" ");
				pclas = pclas[0];			
				pclas = "."+pclas;
		...
		var pid = _qchero(a.parentNode).getAttr('id');
		var pnodename = _qchero(a.parentNode).nodeName.toLowerCase();
			if(pid) pid = "#"+pid; 
				else pid = "";
			if(id) id = "#"+id; 
				else id="";
...
_qchero.parseJSON = function (obj) {
